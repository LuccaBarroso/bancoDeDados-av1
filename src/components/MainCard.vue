...
const avancar = () => {
  ...
  if (isPerPageSet.value === false) {
    loading.value = true
    axios
      .post('http://127.0.0.1:5000/fill-buckets')
      .then((response) => {
...
<template>
  <div class="card">
    <h1>Sistema Banco de Dados</h1>
    <div class="options">
      <button @click="changePage(1)" :class="[curAV === 1 ? 'current' : '', 'btn']">AV1</button>
      <button @click="changePage(2)" :class="[curAV === 2 ? 'current' : '', 'btn']">AV2</button>
    ...
    <div class="items" v-if="curAV === 1">
      ...
      <OutputBuckets
        v-if="isPerPageSet"
        :colisions="infoBuckets.colisions"
        :overflows="infoBuckets.overflows"
        :message="infoBuckets.message"
      />
